...
import { FluxoCaixaCentroDTO } from '@modules/shared/models/fluxo-caixa';
...
@Component({
    selector: 'app-fluxos-fluxo-centro',
    templateUrl: './fluxo-centro.component.html',
    styleUrls: ['fluxo-centro.component.scss'],
  })
  export class FluxoCentroComponent implements AfterViewInit {
    @ViewChild('nomeDTOForm') nomeDTOForm: ElementRef | undefined;
    ColumnMode = ColumnMode;
    statusForm = 1;
    selectionTypeSingle = SelectionType.single;
    submitted = false;
    page!: PageGeneric<FluxoCaixaCentroDTO> | null;
    pesquisa!: GenericPesquisaDTO<FluxoCaixaCentroDTO>;
    pageSize = 20;
    ...
    onSubmit(): void {
      ...
      if (this.createForm.invalid) {
        ...
        this.fluxoService.postOrPutFuxoCentro(values, this.statusForm)
          .subscribe({
            next: (data) => {
              this.dtoToForm(data);
              this.statusForm = 2;
    ...
    onLimpa(): void {
      ...
      this.createForm.get('codigo')?.setValue(null);
      ...
      if (id != null && !isNaN(id) && id > 0 && this.statusForm === 2) {
        ...
        activeModal.result.then((result) => {
          if (result === 'confirm') {
            this.spinner.show('fullSpinner');
            let message = '';
            this.fluxoService.delFluxoCentro(id)
              .subscribe({
                next: async (resp: any) => {
                  this.spinner.hide('fullSpinner');
    ...
    onPesquisa(): void {
      ...
      const searchObj: GenericPesquisaDTO<FluxoCaixaCentroDTO> = {
        object: rawValue,
        pageNumber: 0,
    ...
    onPesquisaItem(itemPesqDTO: any): void {
      this.spinner.show('fullSpinner');
      this.fluxoService.findFluxoCentro(itemPesqDTO)
        .subscribe({
          next: (data) => {
            this.spinner.hide('fullSpinner');
            console.log(data);
    ...
    dtoToForm(dto: FluxoCaixaCentroDTO): void {
      ...
      this.createForm.get('codigo')?.setValue(dto.codigo);
    }
